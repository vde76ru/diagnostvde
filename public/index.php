...
$router->get('/admin/diagnost', function() {
    try {
        \App\Middleware\AuthMiddleware::requireRole('admin');
        define('LAYOUT_RENDERING', true); // Флаг для view файла
        \App\Core\Layout::render('admin/diagnost', []);
    } catch (\Exception $e) {
        error_log("Diagnostics route error: " . $e->getMessage());
        http_response_code(500);
        echo '<h1>Ошибка диагностики</h1><p>' . htmlspecialchars($e->getMessage()) . '</p>';
    }
});
